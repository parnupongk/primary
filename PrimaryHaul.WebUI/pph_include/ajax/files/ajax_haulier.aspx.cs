...
{
    public partial class ajax_haulier : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!string.IsNullOrEmpty(Request.Form["varType"]))
            {
                if (Request.Form["varType"].ToString() == "delete")
                {
                    ...
                    PPHfunction.QueryExecuteNonQuery("delete from  Haulier_Info where Haulier_TaxID='" + Request.Form["varID"].ToString() + "'");
                }
            }
        }
    }

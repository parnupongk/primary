        ...
        <div class="col-md-3">
            <select class="form-control" id="VD" name="VD">
                ...
                <% SqlCommand rs_vd = new SqlCommand(sql_vd, objConn); SqlDataReader obj_vd = rs_vd.ExecuteReader(); while (obj_vd.Read()){ %>
                    <option value="<%= obj_vd["VendorID"].ToString() %>" <% if (str_vd == obj_vd["VendorID"].ToString()) { Response.Write("selected"); } %>><%= obj_vd["Vendor_Name_En"].ToString() %></option>
                <% } obj_vd.Close(); %>
            </select>
        </div>
    </div>
...
<div class="form-group">
    <div class="row">
        ...
        <div class="col-md-7" ><input type="button" value="Preview" class="btn btn-default" onclick="js_submit('WKStart', 'WKEnd', 'DC', 'VD', '1', 'urlSubmit');" id="btn01" />&nbsp;&nbsp;&nbsp;<input type="button" value="Export To Excel" class="btn btn-default" onclick="    js_export('WKStart', 'WKEnd', 'DC', 'VD', '2', 'urlSubmit');" id="btn02" /></div>
    </div>
</div>
